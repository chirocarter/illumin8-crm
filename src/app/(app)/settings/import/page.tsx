...
export default async function ImportPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const skipped = spStr(sp, "skipped");
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {imported && (
        <p className="mb-4 rounded-xl bg-good-soft px-4 py-2.5 text-sm font-medium text-good">
          Imported {imported} record{imported === "1" ? "" : "s"}.
          {skipped && Number(skipped) > 0 && (
            <> Skipped {skipped} duplicate{skipped === "1" ? "" : "s"} already on file.</>
          )}
        </p>
      )}
      ...
      <Card>
        ...
        <form action={importCSV} className="space-y-4 px-5 pb-5">
          ...
          <Field label="CSV file" hint="First row must be headers. Recognized columns for accounts: name, vertical, area, address, website, phone, email, status, source, notes. For contacts: first name, last name, title, phone, email, type, source, notes. Re-importing is safe — rows already on file (same name, phone and email) are skipped.">
